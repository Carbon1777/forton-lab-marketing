...
def test_fetch_weekly_missing_blob_for_current_month_returns_none(monkeypatch):
    """No monthly _country.csv yet → installs=None + понятный blocker error."""
    _set_envs(monkeypatch, mode="raw")
    fake_creds = MagicMock(name="creds")
    fake_client = MagicMock()
    fake_bucket = MagicMock()
    ...
    # No data seen → installs=None + понятный blocker error (renders as «—»).
    assert snap.installs is None
    assert snap.error is not None
    assert "GPlay installs CSV not yet" in snap.error
